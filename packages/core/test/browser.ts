 ...
 *   pnpm test:browser
 *
 * Needs `pnpm data:build` to have written `data/synth/default/test.*.bin`.
 */
...
const synth = `${trainingRoot}/data/synth/default`;
if (!existsSync(`${synth}/test.rows.bin`)) {
  throw new Error(
    `${synth}/test.rows.bin is missing; run \`pnpm data:fetch && pnpm data:build\` first.`,
  );
}
...
try {
  ...
  const result = await page.evaluate(async (SEQUENCES) => {
    ...
    const synth = "/packages/training/data/synth/default";
    const active = "/packages/training/active";
    const rows = new Int16Array(await binary(`${synth}/test.rows.bin`));
    const offsets = new Uint32Array(await binary(`${synth}/test.offsets.bin`));
    ...
    for (let sequence = 0; sequence < parity.sequences; sequence++) {
      const a = rows.subarray(offsets[sequence] * 3, offsets[sequence + 1] * 3);
